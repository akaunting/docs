...
It is advised to start with checking the PoS settings. Make sure that all settings fit your needs. If you are not sure with any setting - leave it as is, with the default value.
...
**Next order number** is used to set the number of next order. It will be incremented automatically, but you can adjust it in case of deleted order or in any other case.
...
**Sale category** is used in an income transaction created for a payment.
...
**Barcode scanner usage** switch controls the search field behaviour. When it is set to "Yes", then the search field will automatically get focus, allowing you to scan goods sequentially, one by one.
...
Also, there are screens for receiving a payment and for printing a receipt.
...
Last added item is highlighted by grey color. You can select another item by clicking on it. Quantity, price and discount of the selected item can be adjusted using the numeric keypad.

### Numeric keypad

Numpad serves 3 goals at once - it allows you to select a customer for an order, switch to receiving a payment from a customer, and quickly adjust quantity, price and discount for items added in the order.
...
Right part of the numpad contains numeric buttons from `0` to `9`, `.` button for decimal point, `delete` button, and radio buttons for mode selection. There are 3 modes: `quantity`, `discount`, `price`. The result of pressing other buttons depends on current mode. The current mode can be identified by green color on one of the "mode" radio buttons.

You may use a physical numpad on your computer keyboard as well as buttons of the onscreen numpad.

`Quantity` mode allows you to change the quantity of the selected item. First press of a numeric button immediately changes the quantity. Let's say the selected item has the quantity of "1", then pressing of the `5` button will change it to "5". Subsequent presses of numeric buttons will add numbers to the quantity. So, in the above case, pressing of the `1` button will change the quantity to "51". Pressing of the `.` and `2` buttons will change the quantity to "51.2". The `delete` button clears the last number. When the quantity is equal to zero, pressing of the `delete` button will remove the selected item from an order.
...
Using the `Price` mode you control a price of the selected item. There are no restrictions of how big or small a price can be, so pressing of the `delete` button when price is zero won't remove the selected item from an order.  

### Orders tabs

Orders tabs sections contains a tab for each unprocessed order and buttons for creating a new order and closing a current order.

![Orders tabs](_images/pos-orders-tabs.png)

Each tab shows time when an order was created. Current order's tab is highlighted by light grey color. You may switch between orders by clicking on their tabs. A number on the left side of each tab shows a serial number of an order, and counting begins when a PoS session starts.

### Categories

Categories section shows all categories your goods belong to and allow you to filter goods by clicking on a category.
...
Your goods are shown in the `Items` section as rectangles that contain price, name and image of an item. If you don't provide an image, then a placeholder image will be used.
...
Clicking the `Payment` button in the numpad will open a screen for receiving a payment for an order.

![Receiving payment](_images/pos-receiving-payment.png)

This screen automatically calculates the total due, remaining amount and change. Using the `Cash` and `Card` buttons you can add as many payments as a customer wants. Newly added payment will have amount equal to the remaining amount. By clicking on a payment you select it and make available for adjusting using a numpad. Current payment is highlighted by white color. Payment can be deleted by pressing on the `X` symbol on its right side.
...
`Receipt` screen shows a preview of a receipt and also allows to print it and send by email. If you selected a customer for an order and that customer has an email, then the `email` field will be automatically filled. Of course, you may provide any other valid email address here.
